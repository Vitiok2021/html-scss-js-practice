...
//  - можна писати значення для застосування кастомного коду
// -root - батьківський елемент всередині якого спостерігати за об'єктом
// -margin -відступ
// -threshold - відсоток показу об'єкта для спрацьовування
// -once - спостерігати лише один раз
...
class ScrollWatcher {
  ...
  scrollWatcherRun() {
    ...
    this.scrollWatcherConstructor(document.querySelectorAll('[]'))
  }
  ...
  getScrollWatcherConfig(paramsWatch) {
    ...
    if (paramsWatch.margin.indexOf('px') < 0 && paramsWatch.margin.indexOf('%') < 0) {
      this.scrollWatcherLogging(`йой, налаштування -margin потрібно задавати в PX або %`)
      return
    }
  ...
  scrollWatcherCallback(entry, observer) {
    ...
    // Якщо є атрибут -once прибираємо стеження
    targetElement.hasAttribute('-once') && entry.isIntersecting ? this.scrollWatcherOff(targetElement, observer) : null
